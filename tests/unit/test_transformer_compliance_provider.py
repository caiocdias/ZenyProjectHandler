...
@pytest.mark.parametrize(
    ("explicit_existing", "inferred_format", "expected_fact", "expected_result"),
    (
        (False, False, False, "NAO_AVALIAVEL"),
        (True, True, True, "NAO_AVALIAVEL"),
    ),
)
def test_transformer_rule_omits_unproven_situation_or_inferred_format(
    ...
    assert bool(applicability) is expected_fact
    assert not any(item.chave == "regiao.poste_transformador_formato" for item in result.fatos)
    finding = _region_finding(result, fixture.region_id, _LARGE_RULE)
    assert finding is not None
    assert finding.resultado.value == expected_result


def test_transformer_rule_omits_correlated_facts_when_region_has_two_poles() -> None:
    fixture = _transformer_fixture(
        equipment_code="-3-75",
        resistance=300,
        post_format="CIRCULAR",
