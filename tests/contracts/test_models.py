...
def _representative_models() -> list[ContractModel]:
    ...
    proposal = ReviewProposalDto(
        ...
        situation_label="Instalar",
        label="Poste 1",
        catalog_label="P-1 — Poste",
        detection_summary="Poste · P-1 — Poste",
        confidence="0.92",
        ...
        overlay=ReviewOverlayDto(
            ...
            geometry=ReviewGeometryDto(
                page_id=PageId(UUIDS[2]),
                kind=ReviewGeometryKind.BOX,
                points=(
                    NormalizedPointDto(x="0.1", y="0.2"),
                    NormalizedPointDto(x="0.4", y="0.6"),
                ),
            ),
    ...
    review = ReviewSessionResponse(
        ...
        regions=(
            AnalysisRegionDto(
                ...
                page_id=PageId(UUIDS[2]),
                label="Região 1",
                location_label="Região 1 · documento.pdf · página 1",
                coordinate_label="Sem coordenada identificada",
                action_summary="Instalar: Poste 1",
                detail_summary="Instalar: Poste 1",
