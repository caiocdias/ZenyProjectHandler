"""Integração da identidade visual do cliente com a barra de tarefas do Windows."""
...
class _PropVariantValue(ctypes.Union):
    _fields_ = [  # noqa: RUF012 - metadado mutável obrigatório da API ctypes
...
def configurar_identidade_aplicativo_windows() -> None:
    ...
    if result != 0:
        raise OSError(
            f"O Windows recusou o identificador do cliente (HRESULT {result & 0xFFFFFFFF:#010x})."
        )
...
def configurar_identidade_janela_windows(
    ...
    *,
    ...
    _definir_propriedades_janela(window_handle, properties)
    cleared = False
    ...
    def clear() -> None:
        ...
        if cleared:
            return
        _definir_propriedades_janela(window_handle, dict.fromkeys(properties))
        cleared = True
...
def _definir_propriedades_janela(
    ...
    _verify_hresult(result, "obter as propriedades da janela")
    ...
    try:
        for name, value in properties.items():
            key = _PropertyKey(_guid(_APP_USER_MODEL_FORMAT_ID), _PROPERTY_KEYS[name])
            variant = _prop_variant(value)
            result = int(set_value(property_store, ctypes.byref(key), ctypes.byref(variant)))
            _verify_hresult(result, f"definir a propriedade {name}")
    finally:
...
def _verify_hresult(result: int, operation: str) -> None:
    if result < 0:
        raise OSError(
            f"Falha ao {operation} na integração com o Windows ({result & 0xFFFFFFFF:#010x})."
        )
